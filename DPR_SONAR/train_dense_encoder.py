...
from dpr_t5.models.biencoder import BiEncoderNllLoss, BiEncoderBatch
from dpr_t5.options import (
    setup_cfg_gpu,
    set_seed,
...
from dpr_t5.utils.conf_utils import BiencoderDatasetsCfg
from dpr_t5.utils.data_utils import (
    ShardedDataIterator,
    Tensorizer,
    MultiSetDataIterator,
    LocalShardedDataIterator,
)
from dpr_t5.utils.dist_utils import all_gather_list
from dpr_t5.utils.model_utils import (
...
class BiEncoderTrainer(object):
    ...
    def validate_nll(self) -> float:
        ...
        for i, samples_batch in enumerate(data_iterator.iterate_ds_data()):
            ...
            biencoder_input, q_attn_mask, ctx_attn_mask = biencoder.create_biencoder_input(
                samples_batch,
                self.tensorizer,
                True,
                num_hard_negatives,
                num_other_negatives,
                shuffle=False,
                retrieve_audio= cfg.retrieve_audio,
                query_audio=cfg.query_audio,
                audio_padding_size=cfg.audio_padding_size,
            )
    ...
    def _train_epoch(
        ...
        for i, samples_batch in enumerate(train_data_iterator.iterate_ds_data(epoch=epoch)):
            ...
            biencoder_batch, q_attn_mask, ctx_attn_mask = biencoder.create_biencoder_input(
                samples_batch,
                self.tensorizer,
                True,
                num_hard_negatives,
            ...
            # get the token to be used for representation selection
            from dpr_t5.utils.data_utils import DEFAULT_SELECTOR
