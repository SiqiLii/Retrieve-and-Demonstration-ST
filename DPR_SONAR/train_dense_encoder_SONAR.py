...
from dpr_sonar.models.biencoder_SONAR import BiEncoderNllLoss, BiEncoderBatch
from dpr_sonar.options import (
    setup_cfg_gpu,
    set_seed,
...
from dpr_sonar.utils.conf_utils import BiencoderDatasetsCfg
from dpr_sonar.utils.data_utils_SONAR import (
    ShardedDataIterator,
    Tensorizer,
    MultiSetDataIterator,
    LocalShardedDataIterator,
)
from dpr_sonar.utils.dist_utils import all_gather_list
...
class BiEncoderTrainer(object):
    ...
    def validate_average_rank(self) -> float:
        ...
        for i, samples_batch in enumerate(data_iterator.iterate_ds_data()):
            ...
            for j, batch_start in enumerate(range(0, bsz, sub_batch_size)):
                ...
                q_ids = (
                    (biencoder_input.question_ids) if j == 0 else (None)
                )
                ...
                #q_attn_mask = self.tensorizer.get_attn_mask(biencoder_input.question_ids,biencoder_input.question_lens)
                #ctx_attn_mask = self.tensorizer.get_attn_mask(biencoder_input.context_ids,biencoder_input.context_lens)
                with torch.no_grad():
                    q_dense, ctx_dense = self.biencoder(
                        q_ids,
                        biencoder_input.question_lens,
                        ctx_ids_batch,
                        biencoder_input.context_lens,
                        encoder_type=encoder_type,
    ...
    def _train_epoch(
        ...
        for i, samples_batch in enumerate(train_data_iterator.iterate_ds_data(epoch=epoch)):
            ...
            # get the token to be used for representation selection
            from dpr_sonar.utils.data_utils_SONAR import DEFAULT_SELECTOR
    ...
    def _load_saved_state(self, saved_state: CheckpointState):
        def state_preprocess(saved_state):
            saved_state_new_q = {
                "question_model.encoder."+key: value for (key, value) in saved_state.model_dict["question_model"].items() 
                    }
            saved_state_new_c = {
                "ctx_model.encoder."+key: value for (key, value) in saved_state.model_dict["ctx_model"].items() 
                    }
            saved_state_new_q.update(saved_state_new_c)

            saved_state_new = CheckpointState(
                                saved_state_new_q,
                                saved_state.tensorizer_dict,
                                saved_state.optimizer_dict,
                                saved_state.scheduler_dict,
                                saved_state.offset,
                                saved_state.epoch,
                                saved_state.encoder_params,
                                )
            return saved_state_new
        saved_state = state_preprocess(saved_state)
...
def _do_biencoder_fwd_pass(
    ...
    #input=move_nested_dict_to_device(input, 'cuda:1')
    q_attn_mask = None # tensorizer.get_attn_mask(input.question_ids,input.question_lens)
    ctx_attn_mask = None # tensorizer.get_attn_mask(input.context_ids,input.context_lens)
    if model.training:
        model_out = model(
            input.question_ids,
            input.question_lens,
...
@hydra.main(config_path="conf", config_name="biencoder_train_cfg_SONAR")
