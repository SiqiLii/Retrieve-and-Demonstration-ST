...
from dpr_sonar.data.tables import Table
from dpr_sonar.utils.data_utils_SONAR import read_data_from_json_files, Dataset

logger = logging.getLogger(__name__)
BiEncoderPassage = collections.namedtuple("BiEncoderPassage", ["text", "title","audio"])
...
def get_dpr_files(source_name) -> List[str]:
    if os.path.exists(source_name) or glob.glob(source_name):
        ...
        from dpr_t5.data.download_data import download
        return download(source_name)


class BiEncoderSample(object):
    query: str  #audio transcript
    query_audio: str #audio file name
...
class JsonQADataset(Dataset):
    ...
    def __getitem__(self, index) -> BiEncoderSample:
        ...
        #print("json_sample", json_sample)
        r = BiEncoderSample()
        r.query = self._process_query(json_sample["question"])
        #r.query = self._process_query(json_sample["question"])
        r.query_audio = self._process_query(json_sample["question_audio"])
...
class JsonLTablesQADataset(Dataset):
    ...
    def __getitem__(self, index) -> BiEncoderSample:
        ...
        r.query_audio=json_sample["question_audio"]
        positive_ctxs = json_sample["positive_ctxs"]
