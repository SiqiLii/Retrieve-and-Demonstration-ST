#nohup python sonar_finetuned_text_text_top10.py > run_tst.out &
...
retrieve_from = term_lines
query = lines_tst_origin

sample_example_from = term_lines #always txt -> used to generate txt file
sample_example_query = lines_tst_origin #always txt -> used to generate txt file

file_prefix='/home/sli/DPR_SONAR/results/tst_term_pairs_dpr_sonar_finetune_q_n_p_text_text_freeze_21'

question_type='text'
ctx_type='text'

cfg = OmegaConf.load("/home/sli/DPR_SONAR/conf/gen_embs_SONAR.yaml")
cfg.question_input_type=question_type
cfg.ctx_input_type=ctx_type

cfg.encoder= OmegaConf.load("/home/sli/DPR_SONAR/conf/encoder/sonar.yaml")
cfg = setup_cfg_gpu(cfg)
cfg.model_file='/export/data2/sli/checkpoints/sonar_dpr_finetune_text_text_freeze_21/dpr_biencoder.29' #'/export/data2/sli/checkpoints/sonar_dpr_finetune/dpr_biencoder.0'
...
with open(file_prefix+'_10.txt','w') as f:
...
with open('/home/sli/term_acc_cal/rareword_in_term.txt', 'r') as file:
    rareword_in_term = [line.strip() for line in file]  # Use list comprehension to read lines and remove newline characters

import spacy
nlp=spacy.load("en_core_web_sm") 

accumulated_total_example_rarewords=[]
for j in range(10):
    lines_to_write=[]
    for i,pair in enumerate(tst_term_pairs):
        lines_to_write.append(sample_example_from[pair[1][j]].strip()+' <SEP> '+sample_example_query[pair[0]])
    if j<=2:
        with open(file_prefix+'_{}st_NN_3.txt'.format(j+1),'w') as g: #
            for sent in lines_to_write:
                g.write(sent)
        print("saved txt files of file prefix{}".format(file_prefix))
    ...
    for i in range(len(examples)):
        ...
        for k,item in enumerate(pred_item_sentence):
            try:
                dictionary[item]
    ...
    word_in_term=0
    for word in total_rarewords:
        if word in rareword_in_term:
            word_in_term=word_in_term+1
    print("num of rare words of is {}".format(len(total_rarewords))) #num of unrecognized lemma
    print("num of rare words that appear in term out of all rare words {}".format(word_in_term)) #num of unrecognized lemma
    ...
    total_example_rarewords=list(total_example_rarewords_set)
    
    accumulated_total_example_rarewords=accumulated_total_example_rarewords+total_example_rarewords
    accumulated_total_example_rarewords_set=set(accumulated_total_example_rarewords)
    accumulated_total_example_rarewords=list(accumulated_total_example_rarewords_set)
    
    print("num of rare words of top {} in example is {}".format(j+1,len(accumulated_total_example_rarewords))) #num of recognized word