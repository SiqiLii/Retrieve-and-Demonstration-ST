import importlib
import sys
sys.path.append('/home/sli/DPR_SONAR/models/sonar_model')
sys.path.append('/home/sli/DPR_SONAR/models/')
def init_speech_t5_bert_biencoder(args, **kwargs):
    if importlib.util.find_spec("transformers") is None:
        raise RuntimeError("Please install transformers lib")
    from .speech_t5_model import get_bert_biencoder_components

    return get_bert_biencoder_components(args, **kwargs)

def init_sonar_bert_biencoder(args, **kwargs):
    if importlib.util.find_spec("transformers") is None:
        raise RuntimeError("Please install transformers lib")
    from .sonar_model import get_bert_biencoder_components
...
BIENCODER_INITIALIZERS = {
    ...
    "speech_t5": init_speech_t5_bert_biencoder,
    "sonar": init_sonar_bert_biencoder,
}
