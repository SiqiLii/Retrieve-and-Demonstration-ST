...
from dpr_t5.models.biencoder_DPR import BiEncoderNllLoss, BiEncoderBatch
...
from dpr_t5.utils.data_utils_DPR import (
    ShardedDataIterator,
    Tensorizer,
...
class BiEncoderTrainer(object):
    ...
    def validate_average_rank(self) -> float:
        ...
        for i, samples_batch in enumerate(data_iterator.iterate_ds_data()):
            ...
            for j, batch_start in enumerate(range(0, bsz, sub_batch_size)):
                ...
                q_attn_mask=q_attn_mask.to('cuda')
                ctx_attn_mask=ctx_attn_mask.to('cuda')
                q_ids=q_ids.to('cuda')
                ctx_ids_batch=ctx_ids_batch.to('cuda')
                q_segments=q_segments.to('cuda')
                ctx_seg_batch=ctx_seg_batch.to('cuda')
    ...
    def _train_epoch(
        ...
        for i, samples_batch in enumerate(train_data_iterator.iterate_ds_data(epoch=epoch)):
            ...
            # get the token to be used for representation selection
            from dpr_t5.utils.data_utils_DPR import DEFAULT_SELECTOR
...
@hydra.main(config_path="conf", config_name="biencoder_train_cfg_DPR")
